...
## 题干
给定一棵树的前序遍历和中序遍历结果，重建这棵二叉树。前序开头是根，中序用于划分左右子树。
...
## 遍历策略
递归建树：前序开头是根；用 HashMap 在中序中定位根下标，左侧是左子树，右侧是右子树，再递归构造。

## 复杂度
时间 O(n)：preorder 中每个节点被取作根一次，inorder 下标通过 HashMap O(1) 定位，整体 O(n)<br>空间 O(n)：HashMap 保存 n 个位置，递归栈最坏 O(n)，平衡树 O(log n)；构造出的树不计入额外空间
