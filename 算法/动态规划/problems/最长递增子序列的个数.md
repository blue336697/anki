...
## 定义状态
dp[i] = {{c1::以 nums[i] 结尾的 LIS 长度}}<br>count[i] = {{c1::以 nums[i] 结尾的 LIS 的方案数}}
> 需要同时维护长度和方案数两个数组

## 转移方程
若 dp[j]+1 &gt; dp[i]：count[i] = {{c1::count[j]}}（重置）<br>若 dp[j]+1 == dp[i]：count[i] = {{c1::count[i] + count[j]}}（累加）
> 更长则重置，等长则累加

## 初始化
dp[i] = {{c1::1}}, count[i] = {{c1::1}}
