...
## 定义状态
dp[i][j][k]：第 i 天，j = {{c1::是否持股(0/1)}}，k = {{c2::已卖出次数(0/1/2)}}
> 新增卖出次数维度 k，从 0 到 2

## 转移方程
不持股已卖k次：max({{c1::dp[i-1][1][k-1]+prices[i]}}, {{c2::dp[i-1][0][k]}})<br>持股已卖k次：max({{c3::dp[i-1][0][k]-prices[i]}}, {{c4::dp[i-1][1][k]}})
> III 中卖出时 k+1（依赖 k-1），买入时 k 不变

## 初始化
dp[0][0][0] = {{c1::0}}, dp[0][1][0] = {{c2::-prices[0]}}<br>dp[0][0][1]=dp[0][0][2]=dp[0][1][1]=dp[0][1][2] = {{c3::MIN_VALUE/2}}
