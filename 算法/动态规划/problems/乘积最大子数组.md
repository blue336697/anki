...
## 定义状态
imax = {{c1::以当前元素结尾的最大乘积}}<br>imin = {{c1::以当前元素结尾的最小乘积}}
> 必须同时维护最大和最小，因为负数相乘会翻转

## 转移方程
if(nums[i]&lt;0) swap({{c1::imax, imin}})<br>imax = max(imax*nums[i], {{c1::nums[i]}})<br>imin = min(imin*nums[i], {{c1::nums[i]}})
> 负负得正：最小值×负数可能变最大值

## 初始化
imax = {{c1::1}}, imin = {{c1::1}}, max = {{c1::Integer.MIN_VALUE}}
