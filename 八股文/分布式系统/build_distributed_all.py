"""Build APKG from distributed-system knowledge notes."""
...
OUTPUT_PATH = REPO_ROOT / "牌组" / "八股文" / "分布式系统" / "分布式系统八股文.apkg"
...
def deck_id_for(name: str) -> int:
    return 2_700_000_000 + sum((i + 1) * ord(ch) for i, ch in enumerate(name)) % 100_000_000
        ...
        for category_dir in sorted(KNOWLEDGE_ROOT.iterdir()):
            ...
            for topic_dir in sorted(category_dir.iterdir()):
                ...
                for md_path in sorted(topic_dir.glob("*.md")):
                    ...
                    deck_name = f"八股文::分布式系统::{category_dir.name}::{title}"
