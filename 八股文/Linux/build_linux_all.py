"""Build APKG from Linux knowledge notes."""
...
OUTPUT_PATH = REPO_ROOT / "牌组" / "八股文" / "Linux" / "Linux八股文.apkg"
...
def deck_id_for(name: str) -> int:
    return 3_400_000_000 + sum((i + 1) * ord(ch) for i, ch in enumerate(name)) % 100_000_000
        ...
        for category_dir in sorted(KNOWLEDGE_ROOT.iterdir()):
            ...
            for topic_dir in sorted(category_dir.iterdir()):
                ...
                for md_path in sorted(topic_dir.glob("*.md")):
                    ...
                    deck_name = f"八股文::Linux::{category_dir.name}::{title}"
