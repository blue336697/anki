"""Build APKG from machine-learning knowledge notes."""
...
OUTPUT_PATH = REPO_ROOT / "牌组" / "八股文" / "机器学习" / "机器学习八股文.apkg"
...
def deck_id_for(name: str) -> int:
    return 2_800_000_000 + sum((i + 1) * ord(ch) for i, ch in enumerate(name)) % 100_000_000
        ...
        for category_dir in sorted(KNOWLEDGE_ROOT.iterdir()):
            ...
            for topic_dir in sorted(category_dir.iterdir()):
                ...
                for md_path in sorted(topic_dir.glob("*.md")):
                    ...
                    deck_name = f"八股文::机器学习::{category_dir.name}::{title}"
