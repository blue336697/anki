"""Build the unified data-structures APKG from knowledge/**/*.md."""
...
OUTPUT_PATH = REPO_ROOT / "牌组" / "八股文" / "数据结构" / "数据结构八股文.apkg"
...
CATEGORY_MAP: dict[str, str] = {
    "基础与线性结构": "八股文::数据结构::基础与线性结构",
    "哈希与概率结构": "八股文::数据结构::哈希与概率结构",
    "内存树结构": "八股文::数据结构::内存树结构",
    "外存索引结构": "八股文::数据结构::外存索引结构",
    "图与集合结构": "八股文::数据结构::图与集合结构",
    "并发与分布式结构": "八股文::数据结构::并发与分布式结构",
}
...
def validate_source(md_path: Path, title: str, sections: list[tuple[str, str]]) -> list[str]:
    ...
    for section_title, body in sections:
        ...
        if q_count != 1 or a_count != 1:
            errors.append(f"{section_title}: expected exactly one Q/A pair, got Q={q_count}, A={a_count}")
        if a_count == 1:
    ...
    try:
        for category_dir in sorted(KNOWLEDGE_ROOT.iterdir()):
            ...
            for topic_dir in sorted(category_dir.iterdir()):
                ...
                if not md_files:
                    validation_errors.append(f"empty topic directory: {topic_dir.relative_to(TOPIC_DIR)}")
                for md_path in md_files:
                    ...
                    if previous and previous != deck_name:
                        validation_errors.append(f"deck id collision {deck_id}: {previous} vs {deck_name}")
