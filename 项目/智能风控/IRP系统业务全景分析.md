...
风控中台 On Premise 的建设需要一个以决策引擎为核心的系统，同时海外市场对决策引擎产品的商业化需求也要求建设一套云原生版本。两方面需求促使建设一套能同时满足 OP 版本和云原生版本的决策引擎核心系统。

#### 目标

1. **易用性**：在保证原有核心功能的前提下，大幅改善用户体验，使业务人员能够轻松配置规则，降低技术门槛
2. **平台化**：作为与业务场景分离的通用决策引擎平台，支持多租户管理，能快速应用到各个业务系统
3. **可视化**：基于浏览器的规则设计器，通过拖拉拽+点击的方式即可实现复杂的业务规则定义
4. **兼容性**：能够在各主流操作系统、中间件、浏览器、数据库上运行和使用
5. **国际化**：支持多国语言切换

#### 原有URule产品存在的问题

1. 向导式的UI界面已经过时，与当前市场上的主流产品相比缺乏竞争力
2. 变量库、常量库、参数库、规则（决策集、决策表、决策树、决策流、评分卡）不能全局共享，用户体验差
3. 规则文件配置在数据库中加密存储，查看源文件只能通过URule提供的功能，不利于日常维护
4. URule设计上耦合性高，做UI交互优化时关联点太多导致改动成本高，研发需要对源码把控度很高
5. 取数部分的功能需要独立出来作为特征衍生平台，避免因决策过程中临时加载三方数据而影响执行效率

#### 开源决策引擎对比

调研了市面上5种主流开源决策引擎（均基于 Apache-2.0 协议）：

| 特性 | Drools | LiteFlow | Easy Rule | Aviator | Rule Engine | URule |
|------|--------|---------|-----------|---------|-------------|-------|
| 开发语言 | Java | Java | Java | Java | 不明确 | Java |
| 开源协议 | Apache | Apache | Apache | Apache | 不明确 | Apache |
| 规则算法 | Rete算法优化 | 不明确 | 不明确 | 不明确 | 不明确 | Rete算法优化 |
| 功能支持 | 规则集、决策表、决策流、评分卡、决策树 | 流程编排、脚本语言 | 简单规则、组合规则 | 表达式求值 | 普通规则、决策集、决策表 | 向导式决策集、脚本式决策集、决策树、决策流、决策表 |
| 脚本语言 | DRL | Java/Groovy/Js/Python/Lua/QLExpress/Aviator | POJO/注解 | 仅支持表达式 | 不支持 | Groovy或MVEL |
| 架构搭建成本 | 高（组件多，需BRMS整套方案） | 低 | 低 | 低 | 低 | 中等 |
| 文档支持 | 英文文档详细，中文文档少 | 不明确 | 不明确 | 不明确 | 不明确 | 详细文档支持 |
| 可视化UI | Drools Workbench | 非官方开源UI项目 | 不支持WEB编辑 | 不支持 | 简陋 | 基于网页的设计器 |
| 适用场景 | 复杂业务逻辑、需BRMS解决方案 | 流程编排、脚本语言灵活 | 简单业务逻辑、快速开发 | 表达式求值 | 小业务、简单规则 | 复杂业务逻辑、个性化规则 |

**各引擎要点**：
- **Drools**：基于Rete算法，功能全面但组件太多，架构搭建成本高，需BRMS整套解决方案
- **LiteFlow**：纯后端框架，偏向流程编排能力，规则通过脚本语言（Java/Groovy/Js/Python/Lua等）实现
- **Easy Rule**：基于POJO开发与注解编程模型，轻量级纯后端框架，不支持WEB编辑
- **Aviator**：高性能轻量级表达式求值引擎，无if-else/do-while等语句，仅支持逻辑/算术/三元/正则表达式
- **Rule Engine**：有可视化界面但极其简陋，仅支持普通规则和决策集、决策表

#### 方案选型

**选型标准**：快速高效、功能完善（含决策集、决策表、决策树、决策流、评分卡可视化构建）、执行效率高、模块化易改造扩展。

| 维度 | 方案一（完全基于URule改造） | 方案二（LiteFlow + URule核心） |
|------|--------------------------|------------------------------|
| UI交互改造 | 改造成最新拖拉拽，需适配原有逻辑，二开成本高 | 重新设计一套，成本较低 |
| 变量/规则共享 | 需从保存、更新、执行解析全方位改造，二开成本很高 | 重新设计存储结构，成本较低 |
| jcr存储 | 原有逻辑关联性强，改造成本较高 | 只用到读取功能，改造成本低 |
| 三方数据独立 | 目前耦合度较低，分割方便 | 重新设计开发 |
| 配套功能 | 比较完善 | 无，需要重新开发 |
| 执行效率 | 基于Rete算法执行效率高 | 采用URule核心，执行效率与URule一致 |
| 决策类型 | 5种全覆盖 | 5种全覆盖 |

**保留URule核心的原因**：
1. **核心能力完善**：URule已提供规则集、决策表、交叉决策表、决策树、评分卡的定义和解析执行能力，短期内二开成本高
2. **Rete算法优势**：高效规则模式匹配，大量复杂规则计算时毫秒级响应，共享条件节点减少重复计算，提高性能和可伸缩性

**最终选型：LiteFlow + URule 核心**。流程编排选择轻量级、执行效率高的 LiteFlow，规则部分保留 URule 核心（解析引擎+优化引擎+执行引擎），前端重新设计。

#### 技术要点验证

| 技术要点 | 验证结果 |
|---------|---------|
| URule 核心代码移植（解析引擎+优化引擎+执行引擎） | ✅ |
| jcr 存储改为数据库存储 | ✅ |
| 决策表单测 | ✅ |
| 决策树单测 | ✅ |
| 决策集单测 | ✅ |
