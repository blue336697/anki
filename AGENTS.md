...
`build_from_mds.py` — 读取 `problems/*.md`，调用 `.Codex/skills/anki-apkg-generator/scripts/apkg_builder.py` 生成 `.apkg` 文件输出到 `牌组/` 目录。
...
- 内容为中文，代码示例使用对应语言标注
- 制作八股文 Anki 卡片时，优先复用原文已有且清晰的配图；只有当知识点属于结构型、流程型或复杂对比型，并且原文没有可用图片、单靠文字很难复习时，才使用 `drawio-skill` 生成 `.drawio` 图并导出 PNG 后在 md 中引用
- 不为纯定义、纯列表、纯结论类知识点额外配图，避免复习时变成看图识别而不是主动回忆
...
- **drawio-skill**：用于缺少原图且确实需要视觉化的八股文知识点，生成流程图、结构图、架构图等 `.drawio`/PNG 资源
